import 'package:blockchain_utils/blockchain_utils.dart';
import 'package:on_chain/serialization/bcs/move/types/types.dart';

class AptosAddress extends MoveAddress
    with CborTagSerializable, Equality
    implements IAddress {
  /// Represents the address "0x0", often used as a null or uninitialized value
  static AptosAddress get zero => AptosAddress("0x0");

  /// Represents the address "0x1", commonly reserved for core framework modules.
  static AptosAddress get one => AptosAddress("0x1");

  /// Represents the address "0x2", typically used for standard library modules.
  static AptosAddress get two => AptosAddress("0x2");

  /// Represents the address "0x3", reserved for system-level functionalities.
  static AptosAddress get three => AptosAddress("0x3");

  /// Represents the address "0x4", also used for predefined system modules.
  static AptosAddress get four => AptosAddress("0x4");

  /// Represents the address "0xA", which may be used for specialized system components
  static AptosAddress get A => AptosAddress("0xA");

  /// The address as a string (in hex format with a "0x" prefix)
  @override
  final String address;
  AptosAddress._(this.address, List<int> bytes) : super(bytes);

  factory AptosAddress(String address) {
    return AptosAddress.fromBytes(AptosAddressUtils.addressToBytes(address));
  }

  factory AptosAddress.deserializeIAddress({
    List<int>? bytes,
    CborObject? object,
  }) {
    final values = CborTagSerializable.decodeTaggedValue(
      identifier: BlockchainNetwork.aptos.identifier,
      cborBytes: bytes,
      cborObject: object,
    );
    return AptosAddress.fromBytes(values.rawValueAt(0));
  }

  /// Factory constructor to create an AptosAddress from raw byte data
  factory AptosAddress.fromBytes(List<int> bytes) {
    final addressBytes = AptosAddressUtils.praseAddressBytes(bytes);
    return AptosAddress._(
      BytesUtils.toHexString(addressBytes, prefix: "0x"),
      addressBytes,
    );
  }

  factory AptosAddress.fromStruct(Map<String, dynamic> json) {
    return AptosAddress.fromBytes(json.valueAsBytes("value"));
  }
  static StructLayout layout({String? property}) {
    return LayoutConst.struct([
      LayoutConst.fixedBlobN(
        AptosAddrConst.addressBytesLength,
        property: "value",
      ),
    ], property: property);
  }

  @override
  Layout<Map<String, dynamic>> createLayout({String? property}) {
    return layout(property: property);
  }

  @override
  Map<String, dynamic> toLayoutStruct() {
    return {"value": value};
  }

  /// address in raw byte format
  List<int> toBytes() {
    return value.clone();
  }

  @override
  String toString() {
    return address;
  }

  @override
  BlockchainNetwork get blockchainNetwork => BlockchainNetwork.aptos;

  @override
  SerializationIdentifier get serializationIdentifier =>
      blockchainNetwork.identifier;

  @override
  List<CborObject?> get serializationItems => [CborBytesValue(value)];

  @override
  List<dynamic> get variables => [address];

  @override
  List<int> encodeAsIAddress() {
    return toCbor().encode();
  }

  @override
  String? get viewType => null;
}
