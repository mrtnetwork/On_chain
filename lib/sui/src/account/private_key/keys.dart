...
class SuiEd25519Account
  ...
  @override
  SuiEd25519Signature signTransaction(
    List<int> txbytes, {
...
class SuiSecp256r1Account
  ...
  @override
  SuiSecp256r1Signature signTransaction(
    List<int> txbytes, {
    bool hashDigest = true,
  }) {
    final digest = SuiCryptoUtils.generateTransactionDigest(
...
class SuiMultisigAccount
  ...
  SuiMultisigAccount({
    required List<SuiBasePrivateKey> privateKeys,
  ...
  SuiMultisigSignature signTransaction(
    List<int> txbytes, {
    List<SuiBasePrivateKey>? signers,
    bool hashDigest = true,
  }) {
    ...
    signers.sort((a, b) {
      ...
      return publicKey.publicKeys
          ...
          .compareTo(
            publicKey.publicKeys.indexWhere((e) => e.publicKey == secoundKey),
          );
    });
    ...
    for (final i in signers) {
      ...
      final index = publicKey.publicKeys.indexWhere(
        (e) => e.publicKey == pubKey,
      );
    ...
    if (weight < publicKey.threshold) {
      throw DartSuiPluginException(
        ...
        details: {
          "threshold": publicKey.threshold.toString(),
