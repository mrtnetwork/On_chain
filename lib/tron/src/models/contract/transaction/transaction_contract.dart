...
class TransactionContract extends TronProtocolBufferImpl {
  ...
  factory TransactionContract.fromJson(Map<String, dynamic> json) {
    final type = TransactionContractType.findByName(json.valueAs("type"));
    final any = Any.fromJson(
      json.valueEnsureAsMap<String, dynamic>("parameter"),
    );
    final int? permissionId = json.valueAsInt("permission_id");

    return TransactionContract(
      type: type,
      parameter: any,
      permissionId: permissionId,
      provider: json.valueAsBytes("provider", encoding: StringEncoding.utf8),
      contractName: json.valueAsBytes(
        "contract_name",
  ...
  factory TransactionContract.deserialize(List<int> bytes) {
    ...
    return TransactionContract(
      type:
          TransactionContractType.findByValue(decode.getField(1)) ??
