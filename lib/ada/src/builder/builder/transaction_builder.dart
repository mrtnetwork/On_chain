...
typedef CbSignAda =
    List<ADABaseTransactionWitness> Function({
      required List<int> digest,
      required ADAAddress address,
    });

typedef CbSignAdaAsync =
    Future<List<ADABaseTransactionWitness>> Function({
      required List<int> digest,
      required ADAAddress address,
    });
...
class ADATransactionBuilder {
  ...
  late final BigInt totalLovelace = utxos.fold(
    BigInt.zero,
    (c, p) => c + p.output.amount.coin,
  );

  late final MultiAsset totalAssets = utxos.fold(
    MultiAsset.empty,
  ...
  TransactionOutput? _changeOutput(ADAAddress addr) {
    ...
    if (lovelace.isNegative) {
      throw ADAPluginException(
        ...
        details: {
          'utxo lovelace': totalLovelace.toString(),
          'output lovelace': _outputs.sumOflovelace.toString(),
        },
      );
    }
    ...
    if (lovelace > BigInt.zero || asset.hasAsset) {
      return TransactionOutput(
        ...
        amount: Value(
          coin: lovelace,
          multiAsset: asset.hasAsset ? asset : null,
        ),
      );
    }
    return null;
  ...
  ADATransaction buildEstimateTx(CbSignAda onSignADA) {
    ...
    final outs =
        _outputs.map((e) {
          if (e.amount.coin == BigInt.zero) {
            return e.copyWith(
              amount: e.amount.copyWith(coin: BinaryOps.maxU64),
            );
          }
          return e;
        }).toList();
  ...
  Future<void> calculateFees(BlockFrostProvider provider) async {
    final protocol = await provider.request(
  ...
  ADATransaction signAndBuildTransaction(CbSignAda onSignADA) {
    _validateAmounts();
    final aux = auxiliaryData;
  ...
  void _validateAmounts() {
    final BigInt depositAmouts = deposits.fold(
      BigInt.zero,
      (previousValue, element) => previousValue + element.deposit,
    );
    final BigInt refundAmounts = refundDeposits.fold(
      BigInt.zero,
      (previousValue, element) => previousValue + element.deposit,
    );
    final lovelace =
