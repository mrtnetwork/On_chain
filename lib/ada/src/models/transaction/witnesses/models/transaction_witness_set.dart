...
class TransactionWitnessSet with InternalCborSerialization {
  ...
  TransactionWitnessSet({
    ...
    this.plutusData,
    this.redeemers,
    this.serializationConfig = const TransactionWitnessSetSerializationConfig(),
  });
  factory TransactionWitnessSet.fromCborBytes(List<int> cborBytes) {
    return TransactionWitnessSet.deserialize(
      CborObject.fromCbor(
        cborBytes,
      ).as<CborMapValue>(operation: "TransactionWitnessSet"),
    );
  }
  factory TransactionWitnessSet.deserialize(CborMapValue cbor) {
    return TransactionWitnessSet(
      vKeys: cbor
          ...
          ?.objectTo<VkeyWitnesses, CborObject>(
            (e) => VkeyWitnesses.deserialize(e),
          ),
      nativeScripts: cbor
          .getIntKeyAs<CborObject?>(1)
          ?.objectTo<NativeScripts, CborObject>(
            (e) => NativeScripts.deserialize(e),
          ),
      bootstraps: cbor
          .getIntKeyAs<CborObject?>(2)
          ?.objectTo<BootstrapWitnesses, CborObject>(
            (e) => BootstrapWitnesses.deserialize(e),
          ),
      plutusScriptsV1: cbor
          .getIntKeyAs<CborObject?>(3)
          ?.objectTo<PlutusScripts, CborObject>(
            (e) => PlutusScripts.deserialize(e),
          ),
      plutusScriptsV2: cbor
          .getIntKeyAs<CborObject?>(6)
          ?.objectTo<PlutusScripts, CborObject>(
            (e) => PlutusScripts.deserialize(e),
          ),
      plutusScriptsV3: cbor
          .getIntKeyAs<CborObject?>(7)
          ?.objectTo<PlutusScripts, CborObject>(
            (e) => PlutusScripts.deserialize(e),
          ),
      plutusData: cbor
          .getIntKeyAs<CborObject?>(4)
          ?.objectTo<PlutusList, CborObject>((e) => PlutusList.deserialize(e)),
      redeemers: cbor
          .getIntKeyAs<CborObject?>(5)
          ?.objectTo<Redeemers, CborObject>((e) => Redeemers.deserialize(e)),
      serializationConfig: TransactionWitnessSetSerializationConfig(
        orderKeys: cbor.value.keys.map((e) => e.value).cast<int>().toList(),
      ),
    );
  }
  ...
  factory TransactionWitnessSet.fromJson(Map<String, dynamic> json) {
    return TransactionWitnessSet(
      vKeys:
          json['vkeys'] == null ? null : VkeyWitnesses.fromJson(json['vkeys']),
      nativeScripts:
          json['native_scripts'] == null
              ? null
