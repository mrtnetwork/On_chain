...
class TransactionOutputs with InternalCborSerialization {
  ...
  TransactionOutputs(
    ...
    this.serializationConfig = const TransactionOutputsSerializationConfig(),
  }) : outputs = outputs.immutable;
  factory TransactionOutputs.deserialize(CborObject cbor) {
    if (cbor.hasType<CborTagValue>()) {
      final tag = cbor.as<CborTagValue>(operation: "outputs");
      final list = tag.asValue<CborIterableObject>(operation: 'outputs');
    ...
    final list = cbor.as<CborIterableObject>(operation: 'outputs');
    return TransactionOutputs(
      list
          .allObjectsAs<CborObject>()
          .map((e) => TransactionOutput.deserialize(e))
