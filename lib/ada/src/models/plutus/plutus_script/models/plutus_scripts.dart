...
class PlutusScripts with InternalCborSerialization {
  ...
  factory PlutusScripts.deserialize(CborObject cbor) {
    if (cbor.hasType<CborTagValue>()) {
      final tag = cbor.as<CborTagValue>(operation: "scripts");
      final list = tag.asValue<CborIterableObject>(operation: 'scripts');
      return PlutusScripts(
        list.allObjectsAs<CborBytesValue>().map((e) => e.value).toList(),
    ...
    final list = cbor.as<CborIterableObject>(operation: 'scripts');
    return PlutusScripts(
      list.allObjectsAs<CborBytesValue>().map((e) => e.value).toList(),
      serializationConfig: PlutusScriptsSerializationConfig(
        encoding: list.encoding,
