...
class PlutusList extends PlutusData {
  ...
  factory PlutusList.deserialize(CborObject cbor) {
    if (cbor.hasType<CborTagValue>()) {
      final tag = cbor.as<CborTagValue>(operation: "PlutusList");
      final data = tag.asValue<CborIterableObject>(operation: "PlutusList");
      return PlutusList(
        data
    ...
    final list = cbor.as<CborIterableObject>(operation: "PlutusList");
    return PlutusList(
      list
          .allObjectsAs<CborObject>()
          .map((e) => PlutusData.deserialize(e))
  ...
  int compareTo(PlutusData other) {
    ...
    if (lenComparison == 0) {
      for (int i = 0; i < value.length; i++) {
        final valueCompare = value
            .elementAt(i)
            .compareTo(other.value.elementAt(i));
