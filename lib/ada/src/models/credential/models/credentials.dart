...
class Credentials with InternalCborSerialization {
  ...
  factory Credentials.deserialize(CborObject cbor) {
    if (cbor.hasType<CborTagValue>()) {
      final tag = cbor.as<CborTagValue>(operation: "Credentials");
      final list = tag.asValue<CborIterableObject>(operation: "Credential");
      return Credentials(
        list
    ...
    final list = cbor.as<CborIterableObject>(operation: "Credential");
    return Credentials(
      list
          .allObjectsAs<CborListValue>()
          .map((e) => Credential.deserialize(e))
  ...
  factory Credentials.fromJson(Map<String, dynamic> json) {
    return Credentials(
      (json["credentials"] as List).map((e) => Credential.fromJson(e)).toList(),
    );
  }
