...
class NativeScripts with InternalCborSerialization {
  ...
  factory NativeScripts.deserialize(CborObject cbor) {
    if (cbor.hasType<CborTagValue>()) {
      final tag = cbor.as<CborTagValue>(operation: "native_scripts");
      final list = tag.asValue<CborIterableObject>(operation: 'native_scripts');
    ...
    final list = cbor.as<CborIterableObject>(operation: 'native_scripts');
    return NativeScripts(
      list
          .allObjectsAs<CborListValue>()
          .map((e) => NativeScript.deserialize(e))
