...
class VotingProposals with InternalCborSerialization {
  ...
  factory VotingProposals.deserialize(CborObject cbor) {
    if (cbor.hasType<CborTagValue>()) {
      final tag = cbor.as<CborTagValue>(operation: "proposals");
      final list = tag.asValue<CborIterableObject>(operation: 'proposals');
    ...
    final list = cbor.as<CborIterableObject>(operation: 'proposals');
    return VotingProposals(
      list
          .allObjectsAs<CborListValue>()
          .map((e) => VotingProposal.deserialize(e))
