...
void _input() {
  test('transaction inputs', () {
    final inp = TransactionInput(
      transactionId: TransactionHash(List<int>.filled(32, 0)),
      index: 0,
    );
    expect(
      inp.serializeHex(),
...
void _output() {
  test('transaction output', () {
    ...
    final output = TransactionOutput(
      address: address,
      amount: Value(coin: BigInt.from(1000000)),
...
void _transaction() {
  ...
  test('transaction without change', () {
    ...
    final output = TransactionOutput(
      ...
      address: address,
      amount: Value(coin: BigInt.from(880000)),
    );
  ...
  test('transaction with refrence', () {
    ...
    final output = TransactionOutput(
      ...
      address: address,
      amount: Value(coin: BigInt.from(500)),
    );
    final changeOutput = TransactionOutput(
      serializationConfig: TransactionOutputSerializationConfig(
        encoding: TransactionOutputCborEncoding.shellyEra,
      ...
      address: changeAddress,
      amount: Value(coin: BigInt.from(999499)),
    );
    final transactionBody = TransactionBody(
      inputs: TransactionInputs(
        [input2],
