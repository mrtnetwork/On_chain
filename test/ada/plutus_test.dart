...
void _constrPlutusDataHash() {
  test('ConstrPlutusData hash', () {
    final constr = ConstrPlutusData(
      alternative: BigInt.zero,
      data: PlutusList([]),
    );
    expect(
      BytesUtils.toHexString(constr.toHash().data),
...
void _plutusData() {
  ...
  test('PlutusData_5', () {
    ...
    expect(
      plutusData.toCbor(sort: true).toCborHex(),
...
void _witnessSetWithPlutus() {
  test('TransactionWitnessSet with plutus data serialization', () {
    PlutusList plutusList = PlutusList(
      [PlutusInteger(BigInt.one)],
      serializationConfig: PlutusListSerializationConfig(
        encoding: CborIterableEncodingType.inDefinite,
      ),
    );
    TransactionWitnessSet witness = TransactionWitnessSet(
...
void _plutusDataBasicJson() {
  test('PlutusData_BasicJson_Serialization_1', () {
    ...
    PlutusData fromJson = PlutusData.fromJsonSchema(
      json: plutusJson,
      schema: PlutusJsonSchema.basicConversions,
    );
    ...
    expect(
      fromJson.toCbor(sort: true).toCborHex(),
...
void _plutusDataDetailedSerialization() {
  test('PlutusData_Detailed_Serialization_1', () {
    ...
    PlutusList fromJson =
        PlutusData.fromJsonSchema(
              json: plutusJson,
              schema: PlutusJsonSchema.detailedSchema,
            )
            as PlutusList;
    expect(
      fromJson.toJsonSchema(
...
void _costModelViewEncoding() {
  test('cont model view encoding', () {
    ...
    final CostModel costModel = CostModel(
      costModelValues.map((e) => BigInt.from(e)).toList(),
    );
    final Costmdls model = Costmdls({Language.plutusV1: costModel});
    expect(
      BytesUtils.toHexString(model.languageViewEncoding().encode()),
  ...
  test('cost model encoding 2', () {
    ...
    final costModel = CostModel(
      contModelValues.map((e) => BigInt.from(e)).toList(),
    );
...
void _scriptHash() {
  test('PlutusData_Script hash', () {
    ...
    final plutusScript = PlutusScript.fromCborBytes(
...
void _redeemer() {
  test('Redeemer_serialization', () {
    final redeemer = Redeemer(
      ...
      exUnits: ExUnits(
        mem: BigInt.from(7000000),
        steps: BigInt.from(3000000000),
      ),
    );
...
void _plutusDataHash() {
  test('plutusDataHash', () {
    final plutusData = PlutusData.fromCborBytes(
      BytesUtils.fromHexString(
    ...
    expect(decode.serialize(), plutusData.serialize());
    expect(
      BytesUtils.toHexString(hash.data),
  ...
  test('PlutusScriptHash_2', () {
    ...
    final redeemer = Redeemer(
      ...
      index: BigInt.zero,
      data: ConstrPlutusData(alternative: BigInt.zero, data: PlutusList([])),
      exUnits: ExUnits(mem: BigInt.from(842996), steps: BigInt.from(246100241)),
    );
    final costModel = CostModel(
      contModelValues.map((e) => BigInt.from(e)).toList(),
    );
  ...
  test('PlutusScriptHash_3', () {
        ...
        ConstrPlutusData(
          ...
          data: PlutusList(
            [
              ...
              PlutusBytes(
                value: BytesUtils.fromHexString(
                  '43727970746F44696E6F3036333039',
                ),
              ),
            ],
            serializationConfig: PlutusListSerializationConfig(
    ...
    final redeemer = Redeemer(
      ...
      exUnits: ExUnits(mem: BigInt.from(61300), steps: BigInt.from(18221176)),
    );
    final costModel = CostModel(
      contModelValues.map((e) => BigInt.from(e)).toList(),
    );
