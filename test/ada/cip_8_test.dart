...
void _test() {
  test('COSESign1', () {
      ...
      headers: COSEHeaders(
        protected: COSEProtectedHeaderMap(
          CborMapValue<CborObject, CborObject>.definite({
            CborIntValue(COSEKeyType.okp.value): CborIntValue(
              COSEAlgorithmId.eddsa.value,
            ),
    ...
    expect(
      BytesUtils.toHexString(builder.toSignMessageBytes()),
...
void _testCOSEKey() {
  test('COSEKey', () {
    final coseKey = COSEKey.deserialize(
      CborObject.fromCborHex(
    ...
    final newKey = COSEKey.fromEd25519Keypair(
      publicKey: publicKey,
      forVerifying: false,
    );
    expect(
...
COSEHeaders _buildHeaderLargeHeader({
  ...
  final header = COSEHeaderMap(
    algorithmId: COSELabelString(algorithmId),
    contentType: COSELabelString(contentType),
    criticality: COSELabels(
      labels: [COSELabelString("one"), COSELabelInt.fromInt(1)],
    ),
    ...
    otherHeaders: {
      ...
      COSELabelString("three"): CborTagValue(
        CborListValue.definite([CborIntValue(1)]),
        [97],
      ),
    },
  );
...
void _serialization() {
  ...
  test('COSECounterSignature', () {
    ...
    expect(
      decode.signatures[0].headers.unprotected.contentType,
      COSELabelString("type"),
    );
    expect(
      decode.signatures[0].headers.unprotected.algorithmId,
      COSELabelString("alg"),
    );
  ...
  test('COSECounterSignature 2', () {
    final sig = COSECounterSignature(
      signatures: [
        _buildSignature(),
        _buildSignature(
          algorithmId: "alg2",
    ...
    expect(
      decode.signatures[1].headers.unprotected.algorithmId,
  ...
  test('COSESign1 2', () {
    ...
    final decodeView = COSESignedMessage.fromUserFacingEncoding(
      decode.toUserFacingEncoding(),
    );

    expect(decodeView, isA<COSESign1>());
    expect(
      (decodeView as COSESign1).headers.unprotected.algorithmId,
      COSELabelString("alg"),
    );
