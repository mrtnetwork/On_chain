...
void _createMetaData() {
  test('createMetaData', () {
    ...
    final decode = SPLTokenMetaDataInitializeLayout.fromBuffer(
      layout.toBytes(),
    );
  ...
  test('createMetaData_3', () {
    final layout = SPLTokenMetaDataUpdateLayout(
      field: SPLTokenMetaDataField.customField(
        keyName: 'website',
        value: 'https://github.com/mrtnetwork',
      ),
    );
    expect(
      layout.toHex(),
...
void _remove() {
  test('remove', () {
    const layout = SPLTokenMetaDataRemoveFieldLayout(
      key: 'https://github.com/mrtnetwork',
      idempotent: false,
    );
    expect(
      layout.toHex(),
    ...
    final decode = SPLTokenMetaDataRemoveFieldLayout.fromBuffer(
      layout.toBytes(),
    );
    expect(layout.toBytes(), decode.toBytes());
  });
...
void _updateAuthority() {
  test('updateAuthority', () {
    ...
    final decode = SPLTokenMetaDataUpdateAuthorityLayout.fromBuffer(
      layout.toBytes(),
    );
    expect(layout.toBytes(), decode.toBytes());
  });
  test('updateAuthority_1', () {
    final newAuthority = SolAddress(
...
void _emit() {
  test('emit', () {
    ...
    expect(
      layout.toHex(),
      'faa6b4fa0d0cb846010100000000000000010200000000000000',
    );
  ...
  test('emit_3', () {
    final layout = SPLTokenMetaDataEmitLayout(
      end: BigInt.from(456456465456465),
    );
  ...
  test('emit_4', () {
    ...
    expect(
      layout.toHex(),
      'faa6b4fa0d0cb846010000000000000000010000000000000000',
    );
    final decode = SPLTokenMetaDataEmitLayout.fromBuffer(layout.toBytes());
    expect(layout.toBytes(), decode.toBytes());
  });
