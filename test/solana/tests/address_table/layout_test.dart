  ...
  test('createLookupTable', () {
    ...
    final tableAddress =
        AddressLookupTableProgramUtils.findAddressLookupTableProgram(
          authority: owner,
          recentSlot: BigInt.from(277769241),
        );
    final layout = AddressLookupCreateLookupTableLayout(
      bumpSeed: tableAddress.bump,
      recentSlot: BigInt.from(277769241),
    );
    expect(layout.toHex(), '00000000196c8e1000000000fe');
    final decode = AddressLookupCreateLookupTableLayout.fromBuffer(
      layout.toBytes(),
    );
...
void _freezeLookupTable() {
  test('freezeLookupTable', () {
    ...
    final decode = AddressLookupFreezeLookupTableLayout.fromBuffer(
      layout.toBytes(),
    );
    expect(decode.toBytes(), layout.toBytes());
  });
...
void _deactivateLookupTable() {
  test('deactivateLookupTable', () {
    ...
    final decode = AddressLookupDeactiveLookupTableLayout.fromBuffer(
      layout.toBytes(),
    );
    expect(decode.toBytes(), layout.toBytes());
  });
...
void _closeLookupTable() {
  test('closeLookupTable', () {
    ...
    final decode = AddressLookupCloseLookupTableLayout.fromBuffer(
      layout.toBytes(),
    );
    expect(decode.toBytes(), layout.toBytes());
  });
