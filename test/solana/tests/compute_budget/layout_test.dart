...
void _requestHeap() {
  test('request heap frame', () {
    ...
    final decode = ComputeBudgetRequestHeapFrameLayout.fromBuffer(
      layout.toBytes(),
    );
...
void _requestUnits() {
  test('request units', () {
    final layout = ComputeBudgetRequestUnitsLayout(
      additionalFee: 1,
      units: 100,
    );
...
void _unitLimit() {
  test('unit limit', () {
    ...
    final decode = ComputeBudgetSetComputeUnitLimitLayout.fromBuffer(
      layout.toBytes(),
    );
    expect(layout.toBytes(), decode.toBytes());
  });
...
void _unitPrice() {
  test('unit price', () {
    ...
    final decode = ComputeBudgetSetComputeUnitPriceLayout.fromBuffer(
      layout.toBytes(),
    );
    expect(layout.toBytes(), decode.toBytes());
  });
