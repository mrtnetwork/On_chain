...
const _owner = SolAddress.unchecked(
  '91KRjTdVaXbdosGDn5AnFczYSApddsFj1SKuShbTUMEB',
);
void main() {
  group('candy machine', () {
    _candyMachine();
...
void _candyMachine() {
  test('add connfig', () {
    final layout = MetaplexCandyMachineAddConfigLinesLayout(
      index: 1,
      configLines: [const ConfigLine(name: 'MRT', uri: 'jafar')],
    );
    expect(
      layout.toHex(),
      'df32e0e39708736a0100000001000000030000004d5254050000006a61666172',
    );
    final decode = MetaplexCandyMachineAddConfigLinesLayout.fromBuffer(
      layout.toBytes(),
    );
  ...
  test('CandyMachineData', () {
    final layout = MetaplexCandyMachineInitializeCandyMachineLayout(
      data: CandyMachineData(
        ...
        isMutable: false,
        creators: [const Creator(address: _owner, verified: false, share: 3)],
      ),
    );
    expect(
      layout.toHex(),
