...
const SolAddress _owner = SolAddress.unchecked(
  'HcEjuQ7Eate3eyNBaZV2cwATcdAH8F7VGygVqkkoqUjf',
);
const SolAddress _account1 = SolAddress.unchecked(
...
void _claimCandyProven() {
  test('claimCandyProven', () {
    ...
    final decode = MetaplexGumdropClaimCandyProvenLayout.fromBuffer(
      layout.toBytes(),
    );
    expect(decode.toBytes(), layout.toBytes());
  });
...
void _claimEdition() {
  test('claimEdition', () {
    ...
    final decode = MetaplexGumdropClaimEditionLayout.fromBuffer(
      layout.toBytes(),
    );
    expect(decode.toBytes(), layout.toBytes());
  });
...
void _closeDistributor() {
  test('closeDistributor', () {
    const layout = MetaplexGumdropCloseDistributorLayout(
      walletBump: 253,
      bump: 1,
    );
    expect(layout.toHex(), 'ca38b48f2e686a7001fd');
    final decode = MetaplexGumdropCloseDistributorLayout.fromBuffer(
      layout.toBytes(),
    );
    expect(decode.toBytes(), layout.toBytes());
  });
...
void _newDistributor() {
  test('newDistributor', () {
    ...
    final decode = MetaplexGumdropNewDistributorLayout.fromBuffer(
      layout.toBytes(),
    );
    expect(decode.toBytes(), layout.toBytes());
  });
...
void _recoverUpdateAuthority() {
  test('recoverUpdateAuthority', () {
    const layout = MetaplexGumdropRecoverUpdateAuthorityLayout(
      bump: 1,
      walletBump: 253,
    );
...
void _claimCount() {
  test('claimCount', () {
    final layout = ClaimCount(count: BigInt.from(111111), claimant: _owner);
    expect(
      layout.toHex(),
