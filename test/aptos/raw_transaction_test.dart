...
void _entryFunction() {
  test("Raw Transaction Entry Function serialization", () {
    ...
    final decodePayload = AptosTransactionPayloadEntryFunction.deserialize(
      payload.toBcs(),
    );
...
void _multisig() {
  test("Raw Transaction Multisig serialization", () {
    ...
    final decodePayload = AptosTransactionPayloadMultisig.deserialize(
      payload.toBcs(),
    );
  ...
  test("Raw Transaction Multisig serialization 2", () {
    ...
    expect(decode.moduleId, function.moduleId);
    expect(decode.typeArgs, function.typeArgs);
    final multisigPayload = AptosMultisigTransactionPayloadEntryFunction(
      entryFunction: function,
    );
    final multisig = AptosMultisig(
      multisigAddress: AptosAddress.one,
      transactionPayload: multisigPayload,
    );
    final payload = AptosTransactionPayloadMultisig(multisig: multisig);
    expect(
      payload.toVariantBcsHex(),
